...
export function findAndReplacePreservingCase(needle, haystack, newWord) {
  ...
  const matchCasing = (oldWord, newWord) => {
    return newWord.substring(0, oldWord.length).split("").map((char, index) => {
      const oldChar = oldWord[index]; return oldChar === oldChar.toUpperCase() ? char.toUpperCase() : char.toLowerCase();
    }).join("");
  };

  const regex = RegExp(needle, "gi");
  return haystack.split(" ").map(needle => regex.test(needle) ? matchCasing(needle, newWord) : needle).join(" ");
}
